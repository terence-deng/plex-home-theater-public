...
Section "Plex" SecPlex
  ...
  File /r /x *.so "${plex_deploy_root}\media\*.*"
  ...
  File /r /x *.so  "${plex_deploy_root}\userdata\*.*"
  
  ;Turn on overwrite for rest of install
  SetOverwrite on
  
  SetOutPath "$INSTDIR\addons"
  File /r "${plex_deploy_root}\addons\*.*"
  ;SetOutPath "$INSTDIR\web"
  ;File /r "${plex_deploy_root}\web\*.*"
...
Section "English" SecLanguageEnglish
  ...
  File /r "${plex_deploy_root}\language\English\*.*"
SectionEnd
...
Section "MediaStream" SecSkinMediaStream
  ...
  File /r "${plex_deploy_root}\addons\skin.mediastream\*.*"
SectionEnd
...
Section "Microsoft Visual C++ 2010 Redistributable Package (x86)" SEC_VCREDIST
  ...
  SetOutPath "$TEMP"
  File "${plex_vcredist_root}\vcredist_x86.exe"
...
Section "DirectX Install" SEC_DIRECTX
  ...
  File "${plex_dependency_root}\dxsetup\dsetup32.dll"
  File "${plex_dependency_root}\dxsetup\DSETUP.dll"
  File "${plex_dependency_root}\dxsetup\dxdllreg_x86.cab"
  File "${plex_dependency_root}\dxsetup\DXSETUP.exe"
  File "${plex_dependency_root}\dxsetup\dxupdate.cab"
  File "${plex_dependency_root}\dxsetup\Jun2010_D3DCompiler_43_x86.cab"
  File "${plex_dependency_root}\dxsetup\Jun2010_d3dx9_43_x86.cab"
