...
void *Cocoa_SwitchRefreshRate(double targetFPS)
{
	...
	// store current mode
	CGDisplayModeRef desktopDisplayMode = activeDisplayMode;
	...
	for (UInt32 i=0; i<CFArrayGetCount(displayModes); i++) 
	{
		...
		{
			NSLog(@"Matching mode: %i x %i @ %i Hz %i bit", width, height, refresh, bitDepth);
	...
	if (CFArrayGetCount(matchingModes) > 0)
	{
		...
		size_t height = CGDisplayModeGetHeight(displayMode);
		size_t width = CGDisplayModeGetWidth(displayMode);
		//		UInt32 integerRate = (UInt32)round(frameRate);
		UInt32 refresh = lrintf(CGDisplayModeGetRefreshRate(displayMode));
		size_t bitDepth = displayBitsPerPixelForMode(displayMode);
		
		if (refresh != currentRefresh)
		{
            NSLog(@"Using %i x %i @ %i Hz %i bit", width, height, refresh, bitDepth);
            // attempt mode switch
            ...
            {
                NSLog(@"Display mode switch failed");
                return nil;
            }	
		}
	}
