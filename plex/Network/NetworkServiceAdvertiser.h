...
class NetworkServiceAdvertiser : public NetworkServiceBase
{
 public:
  ...
  /// Constructor.
  NetworkServiceAdvertiser(io_service& ioService, unsigned short port)
   : NetworkServiceBase(ioService)
   , m_port(port)
  {
    // This is where we'll send notifications.
    m_notifyEndpoint = ip::udp::endpoint(NS_BROADCAST_ADDR, m_port+1);
  }
  ...
  void stop()
  {
    ...
    try { m_socket->send_to(buffer(hello), m_notifyEndpoint); } catch (...) {}
    m_socket->close();
  }
 ...
 private:
  ...
  virtual void handleNetworkChange(const vector<NetworkInterface>& interfaces)
  {
    ...
    // Start up new socket.
    m_socket = udp_socket_ptr(new ip::udp::socket(m_ioService));
    ...
    m_socket->async_receive_from(buffer(m_data, NS_MAX_PACKET_SIZE), m_endpoint, boost::bind(&NetworkServiceAdvertiser::handleRead, this, m_socket, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
  }
  ...
  void broadcastMessage(const udp_socket_ptr& socket, const string& action)
  {
    ...
    try { socket->send_to(buffer(hello), m_notifyEndpoint); } catch (...) {}
  }
  ...
  void handleRead(const udp_socket_ptr& socket, const boost::system::error_code& error, size_t bytes)
  {
    if (!error)
    {
      ...
      try { socket->send_to(buffer(reply), m_endpoint); } 
      catch (...) { wprintf("Error replying to broadcast packet."); }
      
      socket->async_receive_from(buffer(m_data, NS_MAX_PACKET_SIZE), m_endpoint, boost::bind(&NetworkServiceAdvertiser::handleRead, this, socket, boost::asio::placeholders::error, boost::asio::placeholders::bytes_transferred));
    }
    else
    {
  ...
  string createReplyMessage()
  {
    ...
    if (body.empty() == false)
    {
      reply += "Content-Length: " + lexical_cast<string>(body.size()) + CRLF;
    }
  ...
  ip::udp::endpoint m_notifyEndpoint;
