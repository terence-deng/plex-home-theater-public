...
void CFilePlaylist::Process()
{
  if (!m_trackingUrl.IsEmpty())
  {
    CHTTP http;
    CStdString response;
    http.Get(m_trackingUrl, response);
...
{
  CAction action(ACTION_SET_VIDEO_TIME);
...
void CFilePlaylist::ResetDemuxer()
{
  ...
  CAction action(ACTION_RESET_DEMUXER);
  if (g_application.m_pPlayer)
    g_application.m_pPlayer->OnAction(action);  
}
...
bool CFilePlaylist::InsertPlaylistDataToList(CFileItemPtr newPlaylistItem, PLAYLIST::CPlayList* newPlaylist)
{
  ...
  int newPlaylistBandwidth = atoi(newPlaylistItem->GetProperty("m3u8-bandwidth").c_str());
  ...
  newPlaylistData->m_targetDuration = atoi(targetDuration.c_str());
...
bool CFilePlaylist::GetEncryptKey(const CStdString& encryptKeyUri, CStdString& encryptKeyValue)
{
  ...
  CStdString encryptionKeyEncryptedBase64;
  CHTTP().Get(urlStr, encryptionKeyEncryptedBase64);
