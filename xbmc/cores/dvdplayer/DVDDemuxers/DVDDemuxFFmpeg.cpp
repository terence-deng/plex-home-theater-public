...
void ff_avutil_log(void* ptr, int level, const char* format, va_list va)
{
  ...
  if(level >= AV_LOG_DEBUG && g_advancedSettings.m_logLevel <= LOG_LEVEL_DEBUG_FREEMEM)
    return;
...
void CDVDDemuxFFmpeg::GetChapterName(std::string& strChapterName)
{
  ...
  else
  {
      ...
      if(chapterIdx > 0)
      {
        AVChapter* chapter = m_pFormatContext->chapters[chapterIdx-1];
        AVMetadataTag* title = m_dllAvFormat.av_metadata_get(chapter->metadata, "title", NULL, 0);
        if (title && title->value)
          strChapterName = title->value;
      }
    #endif
  }
}
